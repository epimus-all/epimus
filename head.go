...
type Head struct {
	...
	// chunkDiskMapper_ is used to write and read Head chunks to/from disk.(compact mode)
	chunkDiskMapper_ *chunks.ChunkDiskMapper
...
func NewHead(r prometheus.Registerer, l *slog.Logger, wal, wbl *wlog.WL, opts *HeadOptions, stats *HeadStats) (*Head, error) {
	...
	h.chunkDiskMapper_, err = chunks.NewChunkDiskMapper(
		r,
		mmappedChunksDir(opts.ChunkDirRoot+"_"),
		opts.ChunkPool,
		opts.ChunkWriteBufferSize,
		opts.ChunkWriteQueueSize,
	)
