...
func LoadFile(filename string, agentMode bool, logger *slog.Logger) (*Config, error) {
	...
	MetricConfigs = cfg.MetricConfigs
...
type Config struct {
	...
	MetricConfigs     []*MetricConfig `yaml:"metric_configs,omitempty"`
...
type MetricConfig struct {
	...
	UndoRetention int    `yaml:"undo_retention,omitempty"`
}

func IsSole(name string, metrics []*MetricConfig) bool {
...
func GetUndoRetention(name string, metrics []MetricConfig) int {
	for _, metric := range metrics {
		if name == metric.Name {
			return metric.UndoRetention
		}
	}
	return 100
}
