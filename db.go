...
func (db *DB) compactHead(head *RangeHead) error {
	db.compactor.Write0(db.dir, db.c0Block(), db.head, head.MinTime(), head.BlockMaxTime(), nil, true, true)
	uids, err := db.compactor.Write0(db.dir, nil, head, head.MinTime(), head.BlockMaxTime(), nil, true, false)
	if err != nil {
...
func (db *DB) c0Block() *Block {
	db.mtx.RLock()
	defer db.mtx.RUnlock()

	for _, b := range db.Blocks() {
		id := ulid.ULID{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF}
		if b.meta.ULID == id {
			return b
		}
	}
...
func (db *DB) c1Block() *Block {
	db.mtx.RLock()
	defer db.mtx.RUnlock()

	for _, b := range db.Blocks() {
		if b.meta.ULID.String() == "c1" {
			return b
		}
	}
	return nil
