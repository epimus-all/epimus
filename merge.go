...
func isSole(matchers ...*labels.Matcher) bool {
	for _, m := range matchers {
		if m.Type == labels.MatchEqual && m.Name == "__name__" && config.IsSole(m.Value, config.MetricConfigs) {
			return true
		}
	}
	return false
...
func (q *mergeGenericQuerier) Select(ctx context.Context, sortSeries bool, hints *SelectHints, matchers ...*labels.Matcher) genericSeriesSet {
	...
	seriesSets := make([]genericSeriesSet, 0, len(qs))
	var limit int
	if hints != nil {
		limit = hints.Limit
	}
	if !q.concurrentSelect {
		for _, querier := range qs {
