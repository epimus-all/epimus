...
type Compactor interface {
	...
	Write0(dest string, c0block BlockReader, b BlockReader, mint, maxt int64, base *BlockMeta, way bool, sole bool) ([]ulid.ULID, error)
...
func (c *LeveledCompactor) CompactWithBlockPopulator(dest string, dirs []string, open []*Block, blockPopulator BlockPopulator) ([]ulid.ULID, error) {
	...
	err := c.write(dest, meta, blockPopulator, false, false, blocks...)
	if err == nil {
...
func (c *LeveledCompactor) Write(dest string, b BlockReader, mint, maxt int64, base *BlockMeta) ([]ulid.ULID, error) {
	return c.Write0(dest, nil, b, mint, maxt, base, false, false)
}

func (c *LeveledCompactor) Write0(dest string, c0block BlockReader, b BlockReader, mint, maxt int64, base *BlockMeta, way bool, sole bool) ([]ulid.ULID, error) {
	start := time.Now()

	var uid ulid.ULID
	if way {
		uid = ulid.ULID{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF}
	} else {
		uid = ulid.MustNew(ulid.Now(), rand.Reader)
	}
	...
	var err error
	if c0block != nil {
		err = c.write(dest, meta, DefaultBlockPopulator{}, way, sole, b, c0block)
	} else {
		err = c.write(dest, meta, DefaultBlockPopulator{}, way, sole, b)
	}
...
func (c *LeveledCompactor) write(dest string, meta *BlockMeta, blockPopulator BlockPopulator, way bool, sole bool, blocks ...BlockReader) (err error) {
	...
	if err := blockPopulator.PopulateBlock(c.ctx, c.metrics, c.logger, c.chunkPool, c.mergeFunc, blocks, meta, indexw, chunkw, AllSortedPostings, false, false); err != nil {
...
type BlockPopulator interface {
	PopulateBlock(ctx context.Context, metrics *CompactorMetrics, logger *slog.Logger, chunkPool chunkenc.Pool, mergeFunc storage.VerticalChunkSeriesMergeFunc, blocks []BlockReader, meta *BlockMeta, indexw IndexWriter, chunkw ChunkWriter, postingsFunc IndexReaderPostingsFunc, way bool, sole bool) error
}
...
func trim(chks []chunks.Meta, i int) []chunks.Meta {
	var length = 0
	metas := make([]chunks.Meta, 0)
	sizeArray := make([]int, len(chks))
	for j := range chks {
		sizeArray[j] = chks[j].Chunk.NumSamples()
		length += sizeArray[j]
	}
	if length <= i {
		return chks
	...
	var left = length
	var target = false
	for i, chkLength := range sizeArray {
		if left > chkLength {
			left -= chkLength
		} else {
			getBytes(chks[i], left)
			target = true
		}
		if target {
			metas = append(metas, chks[i])
		}
	}
	return metas
}

func getBytes(meta chunks.Meta, index int) {
	var iter chunkenc.Iterator
	meta.Chunk.Iterator(iter)
	var i = 0
	var c chunkenc.Chunk
	for typ := iter.Next(); typ != chunkenc.ValNone; typ = iter.Next() {
		if c == nil {
			c, _ = chunkenc.NewEmptyChunk(typ.ChunkEncoding())
		}
		appender, _ := c.Appender()
		i++
		if i > index {
			appender.Append(iter.At())
		}
	}
	meta.Chunk.Reset(c.Bytes())
}
...
func (c DefaultBlockPopulator) PopulateBlock(ctx context.Context, metrics *CompactorMetrics, logger *slog.Logger, chunkPool chunkenc.Pool, mergeFunc storage.VerticalChunkSeriesMergeFunc, blocks []BlockReader, meta *BlockMeta, indexw IndexWriter, chunkw ChunkWriter, postingsFunc IndexReaderPostingsFunc, way bool, stable bool) (err error) {
	if len(blocks) == 0 {
	...
	for set.Next() {
		...
		s := set.At()
		sole := config.IsSole(s.Labels().Get("__name__"), config.MetricConfigs)
		if way {
			if (stable && sole) || (!stable && !sole) {
				continue
			}
		}
